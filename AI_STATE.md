...
**Último passo:** banda de pés definida pelo usuário — fileiras em 97,5% / 86,25% / 75% da altura (nunca acima de 75%, nunca abaixo de 97,5%).
...
- **Apresentação de batalha (esta sessão, docs/16):** aliados na esquerda (10–42% w) e inimigos na direita (58–90% w); escala relativa ao canvas (herói 19% h, boss 26% h, largura máx 15% w, profundidade 1.04/0.98/0.92); **banda de pés [75%–97,5%] com fileiras em 0.975 / 0.8625 / 0.75** (regra do usuário via `?floordebug`); pré-carga paralela com pool 8 (`battleAssets.ts`); áudio redesenhado em camadas + cama sequenciada 112 BPM. Pendências de perf/áudio (AVIF/WebP, cache imutável, atlas, packs CC0) — avaliar em produção, não regra.
