...
- **UI asset-driven (docs/ui-redesign/01–08, lote-ui-01):** registry `uiRegistry.ts` (UI_KIT, HUB_WORLD com posições normalizadas, HUD_RESOURCES); hub vira cena: praça `rw_env_hub_plaza` + objetos do mundo (torre/arena/honor/passe/correio) + Fonte clicável com WakeBar e burst de sparkles na coleta; HUD com contadores sobre placa asset + nome em cartouche; nav translúcida com sigils/glow; pele global `.panel` (panel_ornate via border-image) e `.cta` (button/button_press sprites) — todo o resto do app vira asset-driven sem mudar lógica; microinterações (float/hover/press/flash de recurso) + prefers-reduced-motion; docs/16 mantém pendências (AVIF/cache/atlas/CC0).
