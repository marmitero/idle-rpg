...
**Último passo:** mecânica de formação 3×3 (9 espaços, 1–5 heróis) — servidor autoritativo, sim com alvo na frente, editor na Relíquias (docs/17).
...
- **Formação 3×3 (esta sessão, docs/17):** 9 espaços por time (frente 0–2, meio 3–5, topo 6–8); `account.formation` é a fonte de verdade (team derivado via `syncFormationTeam` no save); `POST /api/formation` valida 1–5, sem duplicatas, só possuídos; batalhas usam os slots reais (honor/draft por índice); sim prioriza a frente (`slot < 3`) — tanque na frente apanha primeiro; inimigos centralizados (padrões por contagem); editor de formação na Relíquias (aba dedicada "Equipe/Formação" é futura).
